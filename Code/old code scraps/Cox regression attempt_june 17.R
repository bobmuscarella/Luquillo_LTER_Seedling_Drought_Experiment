# Here is the survival script and data. The script is messy, it has in it- our back and forth questions and our failed attempts at plotting. I really want to return to this because I think we should have a three panel figure showing predicted survival vs soil moisture, predicted survival vs densitometer & predicted survival vs starting size but I can’t figure it. Any ideas? The script needs to be modified so the coefficients (and 95% CI) for soil moisture, densitometer & starting size are saved for each species from the best model to be used in plotting with the PCA scores.  

# setwd("C:\\Users\\Matlaga\\Documents\\R")

# For Bob: setwd("/Users/au529793/Downloads")

seedlingdata <- read.table("Seedling drought_survival_May20.txt",header=TRUE,sep="\t")
...
# attach(seedlingdata) # I avoid attaching data because it usually just causes confusion

# the `seedlingdata` object has some weird extra rows at the end... I remove those here:
seedlingdata <- seedlingdata[1:960,]
